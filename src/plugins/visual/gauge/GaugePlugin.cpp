#include "GaugePlugin.h"
#include <QHBoxLayout>
#include <QLabel>
#include <QPainter>
#include <QSignalBlocker>
#include <QVBoxLayout>
#include <algorithm>
#include <cmath>
GaugePlugin::GaugePlugin(QWidget* parent) : IVisualPlugin(parent)
{
    auto* root = new QVBoxLayout(this);
    root->setContentsMargins(8, 8, 8, 8);
    auto* row = new QHBoxLayout;
    m_selector = new QComboBox(this);
    m_selector->setObjectName("gaugeChannelSelector");
    row->addWidget(new QLabel(tr("Channel:"), this));
    row->addWidget(m_selector, 1);
    root->addLayout(row);
    root->addStretch(1);
    connect(m_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, qOverload<>(&GaugePlugin::update));
}
void GaugePlugin::onChannelData(const DataFrame& frame)
{
    bool changed = false;
    for (const auto& sample : frame.channels) {
        if (!std::isfinite(sample.value)) continue;
        const bool isNew = !m_values.contains(sample.index);
        if (isNew && m_values.size() >= 1024) continue;
        const auto label = sample.name.isEmpty() ? QStringLiteral("CH%1").arg(sample.index) : sample.name;
        changed = changed || isNew || m_labels.value(sample.index) != label;
        m_values.insert(sample.index, sample.value);
        m_labels.insert(sample.index, label);
    }
    if (changed) rebuildSelector();
    update();
}
QList<ChannelId> GaugePlugin::subscribedChannels() { return {}; }
qint64 GaugePlugin::historyFrom() { return 0; }
QString GaugePlugin::name() const { return QStringLiteral("Gauge"); }
void GaugePlugin::paintEvent(QPaintEvent* event)
{
    IVisualPlugin::paintEvent(event);
    QPainter painter(this);
    painter.setRenderHint(QPainter::Antialiasing);
    painter.fillRect(rect(), QColor(250, 251, 252));
    if (m_values.isEmpty()) {
        painter.setPen(QColor(95, 99, 104));
        painter.drawText(rect(), Qt::AlignCenter, tr("No numeric channel data"));
        return;
    }
    const ChannelId channel = selectedChannel();
    const double value = m_values.value(channel, 0.0);
    const double clamped = std::clamp(value, 0.0, 100.0);
    const QPoint center(width() / 2, height() / 2 + 40);
    const int radius = std::max(60, std::min(width(), height()) / 3);
    const QRect arc(center.x() - radius, center.y() - radius, radius * 2, radius * 2);
    painter.setPen(QPen(QColor(218, 223, 230), 16, Qt::SolidLine, Qt::RoundCap));
    painter.drawArc(arc, 30 * 16, 120 * 16);
    painter.setPen(QPen(QColor(44, 110, 203), 16, Qt::SolidLine, Qt::RoundCap));
    painter.drawArc(arc, 150 * 16, int(-120.0 * clamped / 100.0 * 16));
    constexpr double pi = 3.14159265358979323846;
    const double angle = (150.0 - 120.0 * clamped / 100.0) * pi / 180.0;
    const QPointF end(center.x() + std::cos(angle) * (radius - 18), center.y() - std::sin(angle) * (radius - 18));
    painter.setPen(QPen(QColor(34, 39, 46), 3));
    painter.drawLine(center, end);
    painter.setBrush(QColor(34, 39, 46));
    painter.drawEllipse(center, 5, 5);
    painter.setFont(QFont(painter.font().family(), 20, QFont::Bold));
    painter.drawText(QRect(0, center.y() + 18, width(), 36), Qt::AlignCenter, QString::number(value, 'f', 2));
    painter.setFont(QFont(painter.font().family(), 10));
    painter.drawText(QRect(0, center.y() + 54, width(), 26), Qt::AlignCenter, m_labels.value(channel));
    painter.drawText(QRect(0, center.y() + 80, width(), 22), Qt::AlignCenter, tr("Needle range: 0..100 (numeric value is not clipped)"));
}
void GaugePlugin::rebuildSelector()
{
    const auto selected = m_selector->currentData();
    const QSignalBlocker blocker(m_selector);
    m_selector->clear();
    auto channels = m_values.keys();
    std::sort(channels.begin(), channels.end());
    for (ChannelId channel : channels) m_selector->addItem(m_labels.value(channel), QVariant::fromValue<qulonglong>(channel));
    const int index = m_selector->findData(selected);
    if (index >= 0) m_selector->setCurrentIndex(index);
}
ChannelId GaugePlugin::selectedChannel() const { return m_selector->currentData().toULongLong(); }
