...
DebugCore::~DebugCore()
{
    // Member destruction is reverse declaration order; clear plugins while the hub is alive.
    invalidateDataPath();
    m_pluginMgr.clear();
}
...
void DebugCore::initialize()
{
    ...
    connect(&m_pluginMgr, &PluginManager::physicalActivated, this, [this](IPhysicalPlugin* p) {
        emit connectionChanged(p->isOpen());
    });
...
void DebugCore::enrich(DataFrame& frame)
{
    for (ChannelSample& sample : frame.channels) {
        ...
        if (!sample.name.isEmpty()) meta.insert("name", sample.name);
        if (!sample.unit.isEmpty()) meta.insert("unit", sample.unit);
        const QVariantMap overrides = m_channelOverrides.value(key).toMap();
...
void DebugCore::publish(const DataFrame& frame)
{
    ...
    enrich(enriched);
    for (const ChannelSample& sample : enriched.channels) {
        if (std::isfinite(sample.value) && !m_ringPool.push(sample.index, {enriched.timestamp_us, sample.value})) {
            if (!m_historyLimitReported) {
                m_historyLimitReported = true;
                emit errorOccurred(tr("Numeric history limit reached; some new samples are not recorded. Clear history to reset."));
            }
        }
    }
...
void DebugCore::sendCommand(const QVariantMap& command)
{
    ...
    const QByteArray bytes = protocol ? protocol->encodeCommand(command) : command.value("bytes").toByteArray();
    if (bytes.isEmpty()) { emit errorOccurred(tr("Invalid command or unsupported command format")); return; }
    if (physical->write(bytes) != bytes.size()) { emit errorOccurred(tr("Command was not accepted into the TX queue")); return; }
    DataFrame tx;
...
{
    QVariantMap result = m_channelMetadata;
...
void DebugCore::replayHistory()
{
    ...
    for (ChannelId channel : m_ringPool.activeChannels()) {
        ...
        for (const auto& sample : samples) {
            ...
            enrich(frame);
            // Replay is display-only: never write back into history or transmit to hardware.
    ...
    if (physical) {
        ...
        m_physicalStatusConnection = connect(physical, &IPhysicalPlugin::statusChanged, this,
            [this, epoch](bool connected) {
                ...
                if (!connected) m_pluginMgr.deactivateAll();
